...
ヒューマノイドロボットにおいて、ヤコビアンは関節速度を重心速度、足速度、手速度などに写像します。
...
ここで $\mathbf{J}^{\dagger}$ はヤコビアンの擬似逆行列です。

ロボットがタスク次元より多くの関節を持つ場合（$n > m$）、システムは**冗長**であり、擬似逆行列は最小ノルム解を与えます。

### 零空間

$\mathbf{J}$ の零空間は、タスク空間出力に影響を与えない運動を表します。副次的な目標を追加できます:

$$
\dot{\mathbf{q}} = \mathbf{J}^{\dagger}\dot{\mathbf{x}}_{des} + (\mathbf{I} - \mathbf{J}^{\dagger}\mathbf{J})\dot{\mathbf{q}}_0
$$

ここで $\dot{\mathbf{q}}_0$ は副次タスクのための任意の関節速度であり、$(\mathbf{I} - \mathbf{J}^{\dagger}\mathbf{J})$ により零空間に射影されます。
